...
struct rt_sensor_config acc_cfg __attribute__((aligned(4)));
struct rt_sensor_config gyr_cfg __attribute__((aligned(4)));
...
void imu_thread_entry(void* parameter)
{
    ...
    while (count--)
    {
        res += _bmi088_get_data((rt_sensor_t) acce_device_t, &acc_test);
        res += _bmi088_get_data((rt_sensor_t) gyro_device_t, &gyr_test);

        if (res >= 2)
            test_flag.flag_bmi088 = RT_TRUE;
