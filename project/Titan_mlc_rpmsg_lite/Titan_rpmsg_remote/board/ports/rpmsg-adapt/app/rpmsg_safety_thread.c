...
#include "tx_api.h"
#elif defined(FSL_RTOS_RTTHREAD)
#include <rtthread.h>
#include <rthw.h>
#endif
 ...
 #else
 #define TEST_THREAD_SAFETY_TASK_STACK_SIZE (10240)
 #endif
...
//     env_init();
 ...
 void rp_task_safety(void)
 {
     ...
     if (result == 0)
     {
         ...
         for (i = 0; i < WORKER_NUMBER; i++)
         {
 ...
 #elif defined(FSL_RTOS_XOS) || defined(FSL_RTOS_THREADX) || defined(FSL_RTOS_RTTHREAD)
             ...
             initParam->ack_q = rpmsg_queue_create(my_rpmsg);
             TEST_ASSERT_MESSAGE(NULL != initParam->ack_q, "Out of memory");
             initParam->ack_ept = rpmsg_lite_create_ept(my_rpmsg, RL_ADDR_ANY, rpmsg_queue_rx_cb, initParam->ack_q);
             TEST_ASSERT_MESSAGE(NULL != initParam->ack_ept, "Out of memory");
 #if defined(SDK_OS_FREE_RTOS)
             result = xTaskCreate(sendRecvTestTask, "THREAD_SAFETY_TASK", TEST_THREAD_SAFETY_TASK_STACK_SIZE, (void *)(initParam), tskIDLE_PRIORITY + 2,
                                  NULL);
...
#elif defined(FSL_RTOS_RTTHREAD)
             rt_thread_t ret = rt_thread_create("safe_task", sendRecvTestTask, (void *)(initParam), TEST_THREAD_SAFETY_TASK_STACK_SIZE, 20, 10);
//            TEST_ASSERT_MESSAGE(RT_NULL == ret, "Could not create worker!");
 ...
 #endif
 {
     ...
     {
         rt_kprintf("."); /* Print a dot, showing progress */
 ...
 int rp_thread_safefy(void)
 {
     ...
     rt_kprintf("                     if it fails, an assert log will be prompted, as follows:\n");
     rt_kprintf("                     Assertion failed: xxx...\n");
