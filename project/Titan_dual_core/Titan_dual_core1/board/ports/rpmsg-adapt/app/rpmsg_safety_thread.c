...
/* Enable safety thread log */
#define RPMSG_APP_LOG   1

#define DRV_DEBUG
#define LOG_TAG             "rpmsg.safeth"
#include <drv_log.h>
...
void responder_task(void)
{
    ...
    while (1)
    {
        ...
        if (0 != ret_value)
        {
            LOG_E("Responder task receive error: %i", ret_value);
        }
        else
        {
#if defined(RPMSG_APP_LOG)
            LOG_I("Responder task received a msg");
            LOG_I("Message: Size=0x%x, CMD = 0x%x, DATA = 0x%x 0x%x 0x%x 0x%x", num_of_received_control_bytes,
