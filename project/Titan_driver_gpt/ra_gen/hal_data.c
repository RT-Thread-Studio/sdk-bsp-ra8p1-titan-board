...
/* Actual period: 0.000262144 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0x10000,
  .duty_cycle_counts = 0x8000, .source_div = (timer_source_div_t) 0, .channel = 1, .p_callback = timer1_callback,
...
/* Actual period: 0.0001 seconds. Actual duty: 50%. */.period_counts = (uint32_t) 0x61a8,
  .duty_cycle_counts = 0x30d4, .source_div = (timer_source_div_t) 0, .channel = 12, .p_callback = NULL,
  /** If NULL then do not add & */
#if defined(NULL)
    .p_context           = NULL,
#else
...
{ .p_ctrl = &g_timer12_ctrl, .p_cfg = &g_timer12_cfg, .p_api = &g_timer_on_gpt };
