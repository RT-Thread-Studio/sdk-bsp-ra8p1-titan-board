...
{ .p_ctrl = &g_transfer0_ctrl, .p_cfg = &g_transfer0_cfg, .p_api =
		&g_transfer_on_dmac };
...
{ .protocol = SPI_FLASH_PROTOCOL_1S_1S_1S, .frame_format =
		...
		.read_command = 0x0B, .read_dummy_cycles = 8, .program_command = 0x02,
		.program_dummy_cycles = 0, .row_load_command = 0x00,
		.row_load_dummy_cycles = 0, .row_store_command = 0x00,
		.row_store_dummy_cycles = 0, .write_enable_command = 0x06,
		.status_command = 0x05, .status_dummy_cycles = 0, },
{ .protocol = SPI_FLASH_PROTOCOL_8D_8D_8D, .frame_format =
		OSPI_B_FRAME_FORMAT_XSPI_PROFILE_1, .latency_mode =
		OSPI_B_LATENCY_MODE_FIXED, .command_bytes = OSPI_B_COMMAND_BYTES_2,
		.address_bytes = SPI_FLASH_ADDRESS_BYTES_4, .address_msb_mask = 0xF0,
		.status_needs_address = true, .status_address = 0x00,
		.status_address_bytes = SPI_FLASH_ADDRESS_BYTES_4, .p_erase_commands =
				&g_ospi_b_command_set_high_speed_erase_table, .read_command =
				0xFDFD, .read_dummy_cycles = 16, .program_command = 0x8484,
		.program_dummy_cycles = 0, .row_load_command = 0x00,
		.row_load_dummy_cycles = 0, .row_store_command = 0x00,
		.row_store_dummy_cycles = 0, .write_enable_command = 0x0606,
		.status_command = 0x0505, .status_dummy_cycles = 8, } };
...
{ .ospi_b_unit = 0, .channel = (ospi_b_device_number_t) 1, .p_timing_settings =
		&g_ospi_b_timing_settings, .p_xspi_command_set = &g_ospi_b_command_set,
...
{ .spi_protocol = SPI_FLASH_PROTOCOL_1S_1S_1S, .read_mode =
		SPI_FLASH_READ_MODE_STANDARD, /* Unused by OSPI_B */
...
{ .p_ctrl = &g_ospi_b_ctrl, .p_cfg = &g_ospi_b_cfg, .p_api =
		&g_ospi_b_on_spi_flash, };
...
/* Actual period: 17.179869184 seconds. Actual duty: 50%. */.period_counts =
		(uint32_t) 0x100000000, .duty_cycle_counts = 0x80000000, .source_div =
...
/** If NULL then do not add & */
    ...
    .p_context           = NULL,
		...
		.p_context = (void*) &NULL,
#endif
		.p_extend = &g_timer_extend, .cycle_end_ipl = (BSP_IRQ_DISABLED),
...
{ .clock = SCI_B_UART_CLOCK_INT, .rx_edge_start =
		SCI_B_UART_START_BIT_FALLING_EDGE, .noise_cancel =
		SCI_B_UART_NOISE_CANCELLATION_DISABLE, .rx_fifo_trigger =
		SCI_B_UART_RX_FIFO_TRIGGER_MAX, .p_baud_setting = &g_uart8_baud_setting,
...
#endif
		.rs485_setting =
		{ .enable = SCI_B_UART_RS485_DISABLE, .polarity =
				SCI_B_UART_RS485_DE_POLARITY_HIGH, .assertion_time = 1,
				.negation_time = 1, } };

/** UART interface configuration */
const uart_cfg_t g_uart8_cfg =
{ .channel = 8, .data_bits = UART_DATA_BITS_8, .parity = UART_PARITY_OFF,
		.stop_bits = UART_STOP_BITS_1, .p_callback = user_uart8_callback,
